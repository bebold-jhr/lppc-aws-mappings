...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/resource/aws_organizations_policy_attachment"
  }
...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::${var.management_account_id}:role/lppc/ResourceAwsOrganizationsPolicyAttachment"
  }
}
...
run "attach_policy" {
  ...
  variables {
    target = var.aws_account_id
    policy = run.prepare_test_setup.policy.id
  }

  assert {
    # Note: role path gets stripped here
    condition     = startswith(data.aws_caller_identity.this.arn, "arn:aws:sts::${var.management_account_id}:assumed-role/ResourceAwsOrganizationsPolicyAttachment")
