...
run "create_bucket" {
  ...
  assert {
    condition     = aws_s3_bucket.this.arn != null
    error_message = "Expected bucket to be created successfully."
  }
}
...
run "add_tag" {
  ...
  assert {
    condition     = aws_s3_bucket.this.tags["createdBy"] == "integration-test"
