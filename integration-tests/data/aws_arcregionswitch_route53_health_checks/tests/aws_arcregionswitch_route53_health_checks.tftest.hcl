...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_arcregionswitch_route53_health_checks"
  }
...
# Perform tests
####
run "fetch_arcregionswitch_route53_health_checks" {
  state_key = "main"
  ...
  variables {
    plan_arn = run.prepare_test_setup.plan.arn
  }
  ...
  assert {
    condition     = data.aws_arcregionswitch_route53_health_checks.this.plan_arn == run.prepare_test_setup.plan.arn
    error_message = "Expected plan ARN to match the one from test setup."
  }
}
