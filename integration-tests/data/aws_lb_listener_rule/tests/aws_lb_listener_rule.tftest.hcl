...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_lb_listener_rule"
  }
...
run "fetch_lb_listener_rule" {
  ...
  variables {
    listener_rule_arn = run.prepare_test_setup.listener_rule.arn
  }
  ...
  assert {
    condition     = data.aws_lb_listener_rule.this.arn == run.prepare_test_setup.listener_rule.arn
    error_message = "Expected listener rule ARN to match the one from test setup."
  }
}
