...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cognito_identity_pool"
  }
...
# Perform tests
####
run "fetch_cognito_identity_pool" {
  state_key = "main"
  ...
  variables {
    identity_pool_name = run.prepare_test_setup.identity_pool.identity_pool_name
  }
  ...
  assert {
    condition     = data.aws_cognito_identity_pool.this.id == run.prepare_test_setup.identity_pool.id
    error_message = "Expected identity pool ID to match the one from test setup."
  }
}
