...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_location_route_calculator"
  }
...
run "fetch_route_calculator" {
  ...
  variables {
    calculator_name = run.prepare_test_setup.route_calculator.calculator_name
  }
  ...
  assert {
    condition     = data.aws_location_route_calculator.this.calculator_name == run.prepare_test_setup.route_calculator.calculator_name
    error_message = "Expected route calculator name to match the one from test setup."
  }
}
