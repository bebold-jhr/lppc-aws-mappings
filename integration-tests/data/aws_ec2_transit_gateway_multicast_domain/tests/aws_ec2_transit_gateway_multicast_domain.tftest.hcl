...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ec2_transit_gateway_multicast_domain"
  }
...
# Perform tests
####
run "fetch_transit_gateway_multicast_domain" {
  state_key = "main"
  ...
  variables {
    multicast_domain_id = run.prepare_test_setup.multicast_domain.id
  ...
  assert {
    condition     = data.aws_ec2_transit_gateway_multicast_domain.this.transit_gateway_id == var.transit_gateway_id
