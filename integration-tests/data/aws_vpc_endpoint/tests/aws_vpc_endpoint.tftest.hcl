...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpc_endpoint"
  }
...
run "fetch_vpc_endpoint" {
  ...
  variables {
    vpc_endpoint_id = run.prepare_test_setup.vpc_endpoint.id
  }
  ...
  assert {
    condition     = data.aws_vpc_endpoint.this.id == run.prepare_test_setup.vpc_endpoint.id
    error_message = "Expected VPC endpoint ID to match the one from test setup."
  }

  assert {
    condition     = data.aws_vpc_endpoint.this.service_name == "com.amazonaws.us-east-1.s3"
    error_message = "Expected service name to be com.amazonaws.us-east-1.s3."
  }
}
