...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_nat_gateways"
  }
...
run "fetch_nat_gateways" {
  ...
  assert {
    condition     = contains(data.aws_nat_gateways.this.ids, run.prepare_test_setup.nat_gateway.id)
    error_message = "Expected NAT Gateway ID to be in the list of NAT Gateways."
  }
}
