...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_appconfig_configuration_profiles"
  }
...
# Perform tests
####
run "fetch_configuration_profiles" {
  state_key = "main"
  ...
  assert {
    condition     = contains(data.aws_appconfig_configuration_profiles.this.configuration_profile_ids, var.configuration_profile_id)
    error_message = "Expected configuration profile ID to be in the list of configuration profiles."
  }
}
