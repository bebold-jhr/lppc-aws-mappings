...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_fis_experiment_templates"
  }
...
# Perform tests
####
run "fetch_fis_experiment_templates" {
  state_key = "main"
  ...
  variables {
    tags = run.prepare_test_setup.experiment_template.tags
  }
  ...
  assert {
    condition     = contains(data.aws_fis_experiment_templates.this.ids, run.prepare_test_setup.experiment_template.id)
    error_message = "Expected experiment template ID to be in the list of returned IDs."
  }
}
