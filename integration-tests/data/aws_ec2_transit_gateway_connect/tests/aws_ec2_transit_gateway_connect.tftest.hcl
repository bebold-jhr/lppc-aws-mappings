...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ec2_transit_gateway_connect"
  }
...
# Perform tests
####
run "fetch_transit_gateway_connect" {
  state_key = "main"
  ...
  variables {
    connect_id         = run.prepare_test_setup.connect.id
  ...
  assert {
    condition     = data.aws_ec2_transit_gateway_connect.this.transit_gateway_id == var.transit_gateway_id
