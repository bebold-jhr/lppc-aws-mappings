...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_servicecatalogappregistry_application"
  }
...
run "fetch_application" {
  ...
  assert {
    condition     = data.aws_servicecatalogappregistry_application.this.arn != ""
    error_message = "Expected application ARN to not be empty."
  }
}
