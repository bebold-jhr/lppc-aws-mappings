...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_devopsguru_notification_channel"
  }
...
# Perform tests
####
run "fetch_devopsguru_notification_channel" {
  state_key = "main"
  ...
  variables {
    notification_channel_id = run.prepare_test_setup.notification_channel.id
  }
  ...
  assert {
    condition     = data.aws_devopsguru_notification_channel.this.sns[0].topic_arn == run.prepare_test_setup.sns_topic.arn
    error_message = "Expected notification channel SNS topic ARN to match the created topic."
  }
}
