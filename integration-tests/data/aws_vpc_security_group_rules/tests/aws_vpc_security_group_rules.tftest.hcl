...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpc_security_group_rules"
  }
...
# Perform tests
####
run "fetch_security_group_rules" {
  state_key = "main"
  ...
  assert {
    condition     = length(data.aws_vpc_security_group_rules.this.ids) > 0
    error_message = "Expected at least one security group rule."
  }
}
