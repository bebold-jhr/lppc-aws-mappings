...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_organizations_organizational_unit_descendant_organizational_units"
  }
...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::${var.management_account_id}:role/lppc/DataAwsOrganizationsOrgUnitDescendantOrganizationalUnits"
  }
}
...
# Perform tests
####
run "fetch_organizational_units" {
  state_key = "main"
  ...
  assert {
    ...
    condition     = startswith(data.aws_caller_identity.this.arn, "arn:aws:sts::${var.management_account_id}:assumed-role/DataAwsOrganizationsOrgUnitDescendantOrganizationalUnits")
  ...
  assert {
    condition     = data.aws_organizations_organizational_unit_descendant_organizational_units.this.children != null && length(data.aws_organizations_organizational_unit_descendant_organizational_units.this.children) > 0
    error_message = "Expected result to contain at least one account."
  }
}
