...
# Perform tests
####
run "fetch_response_headers_policy" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_cloudfront_response_headers_policy.this.name == "Managed-SimpleCORS"
    error_message = "Expected response headers policy name to be Managed-SimpleCORS."
  }
}
