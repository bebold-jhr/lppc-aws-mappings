...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ec2_transit_gateway_route_table_propagations"
  }
...
# Perform tests
####
run "fetch_transit_gateway_route_table_propagations" {
  state_key = "main"
  ...
  variables {
    transit_gateway_route_table_id = run.prepare_test_setup.transit_gateway.propagation_default_route_table_id
  }
  ...
  assert {
    condition     = length(data.aws_ec2_transit_gateway_route_table_propagations.this.ids) > 0
    error_message = "Expected at least one route table propagation."
  }
}
