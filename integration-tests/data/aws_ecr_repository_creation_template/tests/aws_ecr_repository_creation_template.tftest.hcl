...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ecr_repository_creation_template"
  }
...
# Perform tests
####
run "fetch_repository_creation_template" {
  state_key = "main"
  ...
  variables {
    prefix = run.prepare_test_setup.repository_creation_template.prefix
  }
  ...
  assert {
    condition     = contains(data.aws_ecr_repository_creation_template.this.applied_for, "CREATE_ON_PUSH")
    error_message = "Expected applied_for to contain CREATE_ON_PUSH."
  }
}
