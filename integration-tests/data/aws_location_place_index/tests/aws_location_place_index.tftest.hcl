...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_location_place_index"
  }
...
run "fetch_place_index" {
  ...
  variables {
    index_name = run.prepare_test_setup.place_index.index_name
  }
  ...
  assert {
    condition     = data.aws_location_place_index.this.index_name == run.prepare_test_setup.place_index.index_name
    error_message = "Expected place index name to match the one from test setup."
  }
}
