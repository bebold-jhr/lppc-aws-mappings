...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ecr_repository"
  }
...
run "fetch_repository" {
  ...
  variables {
    repository_name = run.prepare_test_setup.repository.name
  }
  ...
  assert {
    condition     = data.aws_ecr_repository.this.name == run.prepare_test_setup.repository.name
    error_message = "Expected ECR repository name to be the same as the repository name from previous step."
  }
}
