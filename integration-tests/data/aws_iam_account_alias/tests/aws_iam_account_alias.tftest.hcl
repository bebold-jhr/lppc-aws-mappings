...
run "fetch_iam_account_alias" {
  ...
  assert {
    condition     = startswith(data.aws_iam_account_alias.this.account_alias, "lppc-")
    error_message = "Expected alias to start with 'lppc' prefix."
  }
}
