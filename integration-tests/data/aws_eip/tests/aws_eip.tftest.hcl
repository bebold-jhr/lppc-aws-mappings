...
run "fetch_eip" {
  ...
  variables {
    allocation_id = run.prepare_test_setup.eip.id
  }
  ...
  assert {
    condition     = data.aws_eip.this.id == run.prepare_test_setup.eip.id
    error_message = "Expected EIP allocation ID to match the one from test setup."
  }
}
