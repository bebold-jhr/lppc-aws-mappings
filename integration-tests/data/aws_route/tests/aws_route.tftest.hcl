...
run "fetch_route" {
  ...
  assert {
    condition     = data.aws_route.this.route_table_id == run.prepare_test_setup.route_table.id
    error_message = "Expected route table ID to match the one from test setup."
  }

  assert {
    condition     = data.aws_route.this.destination_cidr_block == "0.0.0.0/0"
    error_message = "Expected destination CIDR block to be 0.0.0.0/0."
  }
}
