...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ecr_images"
  }
...
run "fetch_images" {
  ...
  assert {
    condition     = data.aws_ecr_images.this.image_ids == null || length(data.aws_ecr_images.this.image_ids) >= 0
    error_message = "Expected ECR images data source to execute successfully."
  }
}
