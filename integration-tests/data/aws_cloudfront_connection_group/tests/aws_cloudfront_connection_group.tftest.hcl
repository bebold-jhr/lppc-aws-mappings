...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cloudfront_connection_group"
  }
...
run "fetch_connection_group" {
  ...
  variables {
    id = run.prepare_test_setup.connection_group.id
  }
  ...
  assert {
    condition     = data.aws_cloudfront_connection_group.this.name == run.prepare_test_setup.connection_group.name
    error_message = "Expected connection group name to match."
  }
}
