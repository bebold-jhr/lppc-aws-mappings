...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_servicecatalog_provisioning_artifacts"
  }
...
# Perform tests
####
run "fetch_provisioning_artifacts" {
  state_key = "main"
  ...
  assert {
    condition     = length(data.aws_servicecatalog_provisioning_artifacts.this.provisioning_artifact_details) > 0
    error_message = "Expected at least one provisioning artifact."
  }
}
