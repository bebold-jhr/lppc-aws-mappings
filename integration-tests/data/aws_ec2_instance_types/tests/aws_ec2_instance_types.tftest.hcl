...
run "fetch_instance_types" {
  ...
  assert {
    condition     = length(data.aws_ec2_instance_types.this.instance_types) > 0
    error_message = "Expected instance types to contain at least one entry."
  }
}
