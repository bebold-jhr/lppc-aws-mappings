...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_networkfirewall_firewall"
  }
...
run "fetch_firewall" {
  ...
  variables {
    firewall_arn = run.prepare_test_setup.firewall.arn
  }
  ...
  assert {
    condition     = data.aws_networkfirewall_firewall.this.arn == run.prepare_test_setup.firewall.arn
    error_message = "Expected firewall ARN to match."
  }
}
