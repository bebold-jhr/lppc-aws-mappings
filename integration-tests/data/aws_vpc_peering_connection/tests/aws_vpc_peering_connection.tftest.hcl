...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpc_peering_connection"
  }
...
run "fetch_peering_connection" {
  ...
  variables {
    peering_connection_id = run.prepare_test_setup.peering_connection.id
  }
  ...
  assert {
    condition     = data.aws_vpc_peering_connection.this.id == run.prepare_test_setup.peering_connection.id
    error_message = "Expected peering connection ID to match the one from test setup."
  }

  assert {
    condition     = data.aws_vpc_peering_connection.this.status == "active"
    error_message = "Expected peering connection status to be active."
  }
}
