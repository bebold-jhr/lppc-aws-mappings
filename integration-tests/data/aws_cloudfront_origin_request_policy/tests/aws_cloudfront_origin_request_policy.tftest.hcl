...
# Perform tests
####
run "fetch_origin_request_policy" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_cloudfront_origin_request_policy.this.name == "Managed-AllViewer"
    error_message = "Expected origin request policy name to be Managed-AllViewer."
  }
}
