...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_sqs_queues"
  }
...
run "fetch_queues" {
  ...
  variables {
    queue_name_prefix = run.prepare_test_setup.queue.name
  }
  ...
  assert {
    condition     = length(data.aws_sqs_queues.this.queue_urls) > 0
    error_message = "Expected at least one SQS queue URL to be returned."
  }
}
