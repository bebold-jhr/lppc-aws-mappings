...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ecs_cluster"
  }
...
run "fetch_cluster" {
  ...
  variables {
    cluster_name = run.prepare_test_setup.cluster.name
  }
  ...
  assert {
    condition     = data.aws_ecs_cluster.this.arn == run.prepare_test_setup.cluster.arn
    error_message = "Expected cluster ARN to match the one from test setup."
  }
}
