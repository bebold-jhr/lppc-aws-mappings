...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_sqs_queue"
  }
...
run "fetch_queue" {
  ...
  variables {
    queue_name = run.prepare_test_setup.queue.name
  }
  ...
  assert {
    condition     = data.aws_sqs_queue.this.name == run.prepare_test_setup.queue.name
    error_message = "Expected SQS queue name to match the queue name from setup."
  }
}
