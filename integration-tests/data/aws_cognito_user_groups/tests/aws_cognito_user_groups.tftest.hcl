...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cognito_user_groups"
  }
...
run "fetch_cognito_user_groups" {
  ...
  assert {
    condition     = length(data.aws_cognito_user_groups.this.groups) > 0
    error_message = "Expected at least one user group."
  }
}
