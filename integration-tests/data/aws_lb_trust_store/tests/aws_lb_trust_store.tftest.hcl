...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_lb_trust_store"
  }
...
run "fetch_lb_trust_store" {
  ...
  variables {
    trust_store_arn = run.prepare_test_setup.trust_store.arn
  }
  ...
  assert {
    condition     = data.aws_lb_trust_store.this.arn == run.prepare_test_setup.trust_store.arn
    error_message = "Expected trust store ARN to match the one from test setup."
  }
}
