...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ssm_parameters_by_path"
  }
...
run "fetch_parameters_by_path" {
  ...
  variables {
    path = "/${split("/", run.prepare_test_setup.parameter.name)[1]}"
  }
  ...
  assert {
    condition     = length(data.aws_ssm_parameters_by_path.this.names) > 0
    error_message = "Expected at least one parameter at the given path."
  }
}
