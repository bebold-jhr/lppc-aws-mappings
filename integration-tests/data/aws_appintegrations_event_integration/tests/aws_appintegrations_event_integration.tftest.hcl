...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_appintegrations_event_integration"
  }
...
run "fetch_event_integration" {
  ...
  variables {
    name = run.prepare_test_setup.event_integration.name
  }
  ...
  assert {
    condition     = data.aws_appintegrations_event_integration.this.name == run.prepare_test_setup.event_integration.name
    error_message = "Expected event integration name to match the one from test setup."
  }

  assert {
    condition     = data.aws_appintegrations_event_integration.this.eventbridge_bus == "default"
    error_message = "Expected eventbridge_bus to be 'default'."
  }
}
