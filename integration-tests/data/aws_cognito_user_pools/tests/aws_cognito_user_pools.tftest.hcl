...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cognito_user_pools"
  }
...
run "fetch_cognito_user_pools" {
  ...
  variables {
    name = run.prepare_test_setup.user_pool.name
  }
  ...
  assert {
    condition     = contains(data.aws_cognito_user_pools.this.ids, run.prepare_test_setup.user_pool.id)
    error_message = "Expected user pool ID to be in the results."
  }
}
