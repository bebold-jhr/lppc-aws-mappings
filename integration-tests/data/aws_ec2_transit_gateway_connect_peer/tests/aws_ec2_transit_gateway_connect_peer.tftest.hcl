...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ec2_transit_gateway_connect_peer"
  }
...
# Perform tests
####
run "fetch_transit_gateway_connect_peer" {
  state_key = "main"
  ...
  variables {
    connect_peer_id = run.prepare_test_setup.connect_peer.id
  }
  ...
  assert {
    condition     = data.aws_ec2_transit_gateway_connect_peer.this.peer_address == "1.1.1.1"
    error_message = "Expected peer address to match."
  }
}
