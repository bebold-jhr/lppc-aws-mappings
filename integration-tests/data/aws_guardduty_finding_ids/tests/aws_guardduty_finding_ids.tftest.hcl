...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_guardduty_finding_ids"
  }
...
# Perform tests
####
run "fetch_guardduty_finding_ids" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_guardduty_finding_ids.this.has_findings == false
    error_message = "Expected no findings for a fresh GuardDuty detector."
  }
}
