...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_servicecatalog_constraint"
  }
...
run "fetch_constraint" {
  ...
  variables {
    constraint_id = run.prepare_test_setup.constraint.id
  }
  ...
  assert {
    condition     = data.aws_servicecatalog_constraint.this.id == run.prepare_test_setup.constraint.id
    error_message = "Expected constraint ID to match the one from test setup."
  }
}
