...
run "fetch_dx_locations" {
  ...
  assert {
    condition     = length(data.aws_dx_locations.this.location_codes) > 0
    error_message = "Expected list of Direct Connect locations to contain at least one entry."
  }
}
