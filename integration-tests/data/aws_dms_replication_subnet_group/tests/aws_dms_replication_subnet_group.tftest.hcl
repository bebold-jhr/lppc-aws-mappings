...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_dms_replication_subnet_group"
  }
...
# Perform tests
####
run "fetch_dms_replication_subnet_group" {
  state_key = "main"
  ...
  variables {
    replication_subnet_group_id = run.prepare_test_setup.replication_subnet_group.replication_subnet_group_id
  }
  ...
  assert {
    condition     = data.aws_dms_replication_subnet_group.this.replication_subnet_group_id == run.prepare_test_setup.replication_subnet_group.replication_subnet_group_id
    error_message = "Expected replication subnet group ID to match the one from test setup."
  }
}
