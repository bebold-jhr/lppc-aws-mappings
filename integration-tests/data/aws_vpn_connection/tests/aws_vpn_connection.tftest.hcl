...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpn_connection"
  }
...
run "fetch_vpn_connection" {
  ...
  variables {
    vpn_connection_id = run.prepare_test_setup.vpn_connection.id
  }
  ...
  assert {
    condition     = data.aws_vpn_connection.this.type == "ipsec.1"
    error_message = "Expected VPN connection type to be ipsec.1."
  }
}
