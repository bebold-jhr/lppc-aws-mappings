...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpclattice_service_network"
  }
...
# Perform tests
####
run "fetch_vpclattice_service_network" {
  state_key = "main"
  ...
  variables {
    service_network_identifier = run.prepare_test_setup.service_network.id
  }
  ...
  assert {
    condition     = data.aws_vpclattice_service_network.this.id == run.prepare_test_setup.service_network.id
    error_message = "Expected VPC Lattice service network ID to match the one from test setup."
  }
}
