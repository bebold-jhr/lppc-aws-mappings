...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_autoscaling_groups"
  }
...
run "fetch_autoscaling_groups" {
  ...
  variables {
    test_id = run.prepare_test_setup.test_id
  }
  ...
  assert {
    condition     = contains(data.aws_autoscaling_groups.this.names, run.prepare_test_setup.autoscaling_group.name)
    error_message = "Expected the created ASG to appear in the autoscaling groups results."
  }
}
