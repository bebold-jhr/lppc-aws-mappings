...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ecr_pull_through_cache_rule"
  }
...
# Perform tests
####
run "fetch_pull_through_cache_rule" {
  state_key = "main"
  ...
  variables {
    ecr_repository_prefix = run.prepare_test_setup.pull_through_cache_rule.ecr_repository_prefix
  }
  ...
  assert {
    condition     = data.aws_ecr_pull_through_cache_rule.this.upstream_registry_url == "public.ecr.aws"
    error_message = "Expected upstream_registry_url to be public.ecr.aws."
  }
}
