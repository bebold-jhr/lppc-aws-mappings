...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_sns_topic"
  }
...
run "fetch_sns_topic" {
  ...
  variables {
    topic_name = run.prepare_test_setup.sns_topic.name
  }
  ...
  assert {
    condition     = data.aws_sns_topic.this.arn == run.prepare_test_setup.sns_topic.arn
    error_message = "Expected SNS topic ARN to match the topic created in the setup."
  }
}
