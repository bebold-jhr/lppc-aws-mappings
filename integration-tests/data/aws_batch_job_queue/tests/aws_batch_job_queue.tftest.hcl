...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_batch_job_queue"
  }
...
run "fetch_batch_job_queue" {
  ...
  variables {
    job_queue_name = run.prepare_test_setup.job_queue.name
  }
  ...
  assert {
    condition     = data.aws_batch_job_queue.this.arn == run.prepare_test_setup.job_queue.arn
    error_message = "Expected job queue ARN to match the one from test setup."
  }
}
