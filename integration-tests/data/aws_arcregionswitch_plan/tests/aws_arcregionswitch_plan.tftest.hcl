...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_arcregionswitch_plan"
  }
...
# Perform tests
####
run "fetch_arcregionswitch_plan" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_arcregionswitch_plan.this.arn == run.prepare_test_setup.plan.arn
