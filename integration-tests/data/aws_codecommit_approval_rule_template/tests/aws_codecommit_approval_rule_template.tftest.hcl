...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_codecommit_approval_rule_template"
  }
...
# Perform tests
####
run "fetch_approval_rule_template" {
  state_key = "main"
  ...
  variables {
    approval_rule_template_name = run.prepare_test_setup.approval_rule_template.name
  }
  ...
  assert {
    condition     = data.aws_codecommit_approval_rule_template.this.name == run.prepare_test_setup.approval_rule_template.name
    error_message = "Expected approval rule template name to match the one from test setup."
  }
}
