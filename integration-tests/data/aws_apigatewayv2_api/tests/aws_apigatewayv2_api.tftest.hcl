...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_apigatewayv2_api"
  }
...
run "fetch_api" {
  ...
  variables {
    api_id = run.prepare_test_setup.api.id
  }
  ...
  assert {
    condition     = data.aws_apigatewayv2_api.this.id == run.prepare_test_setup.api.id
    error_message = "Expected API ID to match the one from test setup."
  }
}
