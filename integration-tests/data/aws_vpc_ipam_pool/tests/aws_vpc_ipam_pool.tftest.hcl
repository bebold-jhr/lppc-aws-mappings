...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpc_ipam_pool"
  }
...
run "fetch_vpc_ipam_pool" {
  ...
  assert {
    condition     = data.aws_vpc_ipam_pool.this.id == run.prepare_test_setup.ipam_pool.id
    error_message = "Expected IPAM Pool ID to match the one from test setup."
  }
}
