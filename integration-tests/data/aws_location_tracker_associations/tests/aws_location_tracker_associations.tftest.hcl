...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_location_tracker_associations"
  }
...
# Perform tests
####
run "fetch_tracker_associations" {
  state_key = "main"
  ...
  assert {
    condition     = contains(data.aws_location_tracker_associations.this.consumer_arns, run.prepare_test_setup.geofence_collection.collection_arn)
    error_message = "Expected consumer ARNs to contain the geofence collection ARN from test setup."
  }
}
