...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpc_security_group_rule"
  }
...
run "fetch_security_group_rule" {
  ...
  variables {
    security_group_rule_id = run.prepare_test_setup.security_group_rule.id
  }
  ...
  assert {
    condition     = data.aws_vpc_security_group_rule.this.security_group_rule_id == run.prepare_test_setup.security_group_rule.id
    error_message = "Expected security group rule ID to match the one from test setup."
  }

  assert {
    condition     = data.aws_vpc_security_group_rule.this.from_port == 443
    error_message = "Expected from_port to be 443."
  }
}
