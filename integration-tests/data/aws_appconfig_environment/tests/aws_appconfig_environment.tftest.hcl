...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_appconfig_environment"
  }
...
run "fetch_environment" {
  ...
  assert {
    condition     = data.aws_appconfig_environment.this.arn == run.prepare_test_setup.environment.arn
    error_message = "Expected environment ARN to match the one from test setup."
  }

  assert {
    condition     = data.aws_appconfig_environment.this.name == run.prepare_test_setup.environment.name
    error_message = "Expected environment name to match the one from test setup."
  }
}
