...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_api_gateway_authorizers"
  }
...
run "fetch_authorizers" {
  ...
  assert {
    condition     = contains(data.aws_api_gateway_authorizers.this.ids, run.prepare_test_setup.authorizer.id)
    error_message = "Expected authorizer ID to be in the list of authorizers."
  }
}
