...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ecs_container_definition"
  }
...
# Perform tests
####
run "fetch_container_definition" {
  state_key = "main"
  ...
  variables {
    task_definition_arn = run.prepare_test_setup.task_definition.arn
  }
  ...
  assert {
    condition     = startswith(data.aws_ecs_container_definition.this.image, "public.ecr.aws/docker/library/nginx")
    error_message = "Expected container image to be nginx."
  }
}
