...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_api_gateway_api_key"
  }
...
run "fetch_api_key" {
  ...
  variables {
    api_key_id = run.prepare_test_setup.api_key.id
  }
  ...
  assert {
    condition     = data.aws_api_gateway_api_key.this.id == run.prepare_test_setup.api_key.id
    error_message = "Expected API key ID to match the one from test setup."
  }
}
