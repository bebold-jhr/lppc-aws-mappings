...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_backup_report_plan"
  }
...
run "fetch_backup_report_plan" {
  ...
  variables {
    report_plan_name = run.prepare_test_setup.backup_report_plan.name
  }
  ...
  assert {
    condition     = data.aws_backup_report_plan.this.name == run.prepare_test_setup.backup_report_plan.name
    error_message = "Expected backup report plan name to match."
  }
}
