...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cloudhsm_v2_cluster"
  }
...
run "fetch_cloudhsm_v2_cluster" {
  ...
  variables {
    cluster_id = run.prepare_test_setup.cluster.cluster_id
  }
  ...
  assert {
    condition     = data.aws_cloudhsm_v2_cluster.this.cluster_id == run.prepare_test_setup.cluster.cluster_id
    error_message = "Expected cluster ID to match the one from test setup."
  }
}
