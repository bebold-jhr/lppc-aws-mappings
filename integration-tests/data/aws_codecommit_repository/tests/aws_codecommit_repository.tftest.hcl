...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_codecommit_repository"
  }
...
run "fetch_repository" {
  ...
  variables {
    repository_name = run.prepare_test_setup.repository.repository_name
  }
  ...
  assert {
    condition     = data.aws_codecommit_repository.this.repository_name == run.prepare_test_setup.repository.repository_name
    error_message = "Expected repository name to match the one from test setup."
  }
}
