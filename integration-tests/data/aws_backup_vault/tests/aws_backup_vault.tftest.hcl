...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_backup_vault"
  }
...
run "fetch_backup_vault" {
  ...
  variables {
    vault_name = run.prepare_test_setup.backup_vault.name
  }
  ...
  assert {
    condition     = data.aws_backup_vault.this.name == run.prepare_test_setup.backup_vault.name
    error_message = "Expected backup vault name to match."
  }
}
