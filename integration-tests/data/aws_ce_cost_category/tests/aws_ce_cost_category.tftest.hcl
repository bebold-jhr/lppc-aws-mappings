...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ce_cost_category"
  }
...
run "fetch_cost_category" {
  ...
  variables {
    cost_category_arn = run.prepare_test_setup.cost_category.arn
  }
  ...
  assert {
    condition     = data.aws_ce_cost_category.this.cost_category_arn == run.prepare_test_setup.cost_category.arn
    error_message = "Expected cost category ARN to match the one from test setup."
  }
}
