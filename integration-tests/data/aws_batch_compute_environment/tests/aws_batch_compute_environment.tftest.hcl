...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_batch_compute_environment"
  }
...
# Perform tests
####
run "fetch_batch_compute_environment" {
  state_key = "main"
  ...
  variables {
    compute_environment_name = run.prepare_test_setup.compute_environment.compute_environment_name
  }
  ...
  assert {
    condition     = data.aws_batch_compute_environment.this.arn == run.prepare_test_setup.compute_environment.arn
    error_message = "Expected compute environment ARN to match the one from test setup."
  }
}
