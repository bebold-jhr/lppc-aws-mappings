...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_elastic_beanstalk_application"
  }
...
run "fetch_application" {
  ...
  assert {
    condition     = data.aws_elastic_beanstalk_application.this.name == run.prepare_test_setup.application.name
    error_message = "Expected application name to match the setup application."
  }
}
