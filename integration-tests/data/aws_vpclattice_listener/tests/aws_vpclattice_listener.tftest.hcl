...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpclattice_listener"
  }
...
run "fetch_vpclattice_listener" {
  ...
  variables {
    service_identifier  = run.prepare_test_setup.service.id
    listener_identifier = run.prepare_test_setup.listener.listener_id
  }
  ...
  assert {
    condition     = data.aws_vpclattice_listener.this.listener_id == run.prepare_test_setup.listener.listener_id
    error_message = "Expected VPC Lattice listener ID to match the one from test setup."
  }
}
