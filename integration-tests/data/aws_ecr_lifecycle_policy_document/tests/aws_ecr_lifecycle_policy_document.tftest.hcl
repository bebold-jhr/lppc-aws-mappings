...
# Perform tests
####
run "fetch_lifecycle_policy_document" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_ecr_lifecycle_policy_document.this.json != ""
    error_message = "Expected lifecycle policy document JSON to be non-empty."
  }
}
