...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_globalaccelerator_accelerator"
  }
...
# Perform tests
####
run "fetch_globalaccelerator_accelerator" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_globalaccelerator_accelerator.this.name == run.prepare_test_setup.accelerator.name
    error_message = "Expected accelerator name to match the one from test setup."
  }
}
