...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_sfn_state_machine_versions"
  }
...
# Perform tests
####
run "fetch_state_machine_versions" {
  state_key = "main"
  ...
  assert {
    condition     = length(data.aws_sfn_state_machine_versions.this.statemachine_versions) > 0
    error_message = "Expected at least one state machine version."
  }
}
