...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_budgets_budget"
  }
...
run "fetch_budget" {
  ...
  variables {
    name = run.prepare_test_setup.budget.name
  }
  ...
  assert {
    condition     = data.aws_budgets_budget.this.name == run.prepare_test_setup.budget.name
    error_message = "Expected budget name to match the one from test setup."
  }
}
