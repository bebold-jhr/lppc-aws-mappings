...
run "fetch_solution_stack" {
  ...
  assert {
    condition     = data.aws_elastic_beanstalk_solution_stack.this.name != null && data.aws_elastic_beanstalk_solution_stack.this.name != ""
    error_message = "Expected solution stack name to be non-empty."
  }
}
