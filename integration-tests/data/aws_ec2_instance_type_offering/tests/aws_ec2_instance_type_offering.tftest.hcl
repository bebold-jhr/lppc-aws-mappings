...
# Perform tests
####
run "fetch_instance_type_offering" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_ec2_instance_type_offering.this.instance_type == "t2.micro"
    error_message = "Expected instance type offering to be t2.micro."
  }
}
