...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpc_peering_connections"
  }
...
run "fetch_peering_connections" {
  ...
  variables {
    requester_vpc_id = run.prepare_test_setup.requester_vpc.id
  }
  ...
  assert {
    condition     = length(data.aws_vpc_peering_connections.this.ids) > 0
    error_message = "Expected at least one peering connection."
  }

  assert {
    condition     = contains(data.aws_vpc_peering_connections.this.ids, run.prepare_test_setup.peering_connection.id)
    error_message = "Expected peering connection from setup to be in the results."
  }
}
