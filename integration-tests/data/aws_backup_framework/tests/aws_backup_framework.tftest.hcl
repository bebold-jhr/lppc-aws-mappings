...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_backup_framework"
  }
...
run "fetch_backup_framework" {
  ...
  variables {
    framework_name = run.prepare_test_setup.backup_framework.name
  }
  ...
  assert {
    condition     = data.aws_backup_framework.this.name == run.prepare_test_setup.backup_framework.name
    error_message = "Expected backup framework name to match."
  }
}
