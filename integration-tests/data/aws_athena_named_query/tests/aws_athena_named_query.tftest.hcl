...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_athena_named_query"
  }
...
run "fetch_named_query" {
  ...
  variables {
    name = run.prepare_test_setup.named_query.name
  }
  ...
  assert {
    condition     = data.aws_athena_named_query.this.name == run.prepare_test_setup.named_query.name
    error_message = "Expected named query name to match."
  }
}
