...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_wafv2_web_acl"
  }
...
run "fetch_web_acl" {
  ...
  variables {
    name = run.prepare_test_setup.web_acl.name
  }
  ...
  assert {
    condition     = data.aws_wafv2_web_acl.this.name == run.prepare_test_setup.web_acl.name
    error_message = "Expected WAFv2 web ACL name to match the one from test setup."
  }
}
