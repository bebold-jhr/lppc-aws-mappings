...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_grafana_workspace"
  }
...
run "fetch_grafana_workspace" {
  ...
  variables {
    workspace_id = run.prepare_test_setup.grafana_workspace.id
  }
  ...
  assert {
    condition     = data.aws_grafana_workspace.this.arn == run.prepare_test_setup.grafana_workspace.arn
    error_message = "Expected workspace ARN to match the one from test setup."
  }
}
