...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ssm_parameter"
  }
...
run "fetch_parameter" {
  ...
  variables {
    parameter_name = run.prepare_test_setup.parameter.name
  }
  ...
  assert {
    condition     = data.aws_ssm_parameter.this.name == run.prepare_test_setup.parameter.name
    error_message = "Expected parameter name to match the created parameter."
  }
}
