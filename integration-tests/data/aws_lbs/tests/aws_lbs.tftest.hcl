...
run "fetch_lbs" {
  ...
  variables {
    tag_value = run.prepare_test_setup.lb.tags.LppcTest
  }
  ...
  assert {
    condition     = contains(data.aws_lbs.this.arns, run.prepare_test_setup.lb.arn)
    error_message = "Expected load balancers list to contain the ARN from test setup."
  }
}
