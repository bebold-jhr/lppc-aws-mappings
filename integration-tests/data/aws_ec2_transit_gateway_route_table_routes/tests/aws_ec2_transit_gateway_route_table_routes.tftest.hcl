...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ec2_transit_gateway_route_table_routes"
  }
...
# Perform tests
####
run "fetch_transit_gateway_route_table_routes" {
  state_key = "main"
  ...
  assert {
    condition     = length(data.aws_ec2_transit_gateway_route_table_routes.this.routes) > 0
    error_message = "Expected at least one active route in the route table."
  }
}
