...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_location_map"
  }
...
run "fetch_map" {
  ...
  variables {
    map_name = run.prepare_test_setup.map.map_name
  }
  ...
  assert {
    condition     = data.aws_location_map.this.map_name == run.prepare_test_setup.map.map_name
    error_message = "Expected map name to match the one from test setup."
  }
}
