...
run "fetch_hosted_zone" {
  ...
  assert {
    condition     = data.aws_elastic_beanstalk_hosted_zone.this.id != null && data.aws_elastic_beanstalk_hosted_zone.this.id != ""
    error_message = "Expected hosted zone ID to be non-empty."
  }
}
