...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_auditmanager_control"
  }
...
# Perform tests
####
run "fetch_auditmanager_control" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_auditmanager_control.this.id != ""
    error_message = "Expected Audit Manager control ID to not be empty."
  }

  assert {
    condition     = data.aws_auditmanager_control.this.type == "Standard"
    error_message = "Expected Audit Manager control type to be Standard."
  }
}
