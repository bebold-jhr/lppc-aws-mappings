...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_instance"
  }
...
run "fetch_instance" {
  ...
  assert {
    condition     = data.aws_instance.this.id == run.prepare_test_setup.instance.id
    error_message = "Expected instance ID to match."
  }

  assert {
    condition     = data.aws_instance.this.instance_type == "t3.micro"
