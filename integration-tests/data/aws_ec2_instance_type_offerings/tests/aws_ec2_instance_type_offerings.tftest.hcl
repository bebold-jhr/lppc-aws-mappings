...
# Perform tests
####
run "fetch_instance_type_offerings" {
  state_key = "main"
  ...
  assert {
    condition     = length(data.aws_ec2_instance_type_offerings.this.instance_types) > 0
    error_message = "Expected instance type offerings to contain at least one entry."
  }
}
