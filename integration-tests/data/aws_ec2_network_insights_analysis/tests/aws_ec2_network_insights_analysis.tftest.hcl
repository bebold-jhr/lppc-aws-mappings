...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ec2_network_insights_analysis"
  }
...
# Perform tests
####
run "fetch_network_insights_analysis" {
  state_key = "main"
  ...
  variables {
    analysis_id = run.prepare_test_setup.analysis.id
    path_id     = run.prepare_test_setup.path.id
  }
  ...
  assert {
    condition     = data.aws_ec2_network_insights_analysis.this.network_insights_path_id == var.path_id
    error_message = "Expected Network Insights Analysis path ID to match the one from test setup."
  }
}
