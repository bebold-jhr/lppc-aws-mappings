...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_servicecatalog_portfolio_constraints"
  }
...
# Perform tests
####
run "fetch_portfolio_constraints" {
  state_key = "main"
  ...
  variables {
    portfolio_id = run.prepare_test_setup.portfolio.id
  }
  ...
  assert {
    condition     = data.aws_servicecatalog_portfolio_constraints.this.portfolio_id == run.prepare_test_setup.portfolio.id
    error_message = "Expected portfolio_id to match the one from test setup."
  }
}
