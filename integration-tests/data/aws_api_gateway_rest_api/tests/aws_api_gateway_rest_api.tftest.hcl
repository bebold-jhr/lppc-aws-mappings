...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_api_gateway_rest_api"
  }
...
run "fetch_rest_api" {
  ...
  variables {
    rest_api_name = run.prepare_test_setup.rest_api.name
  }
  ...
  assert {
    condition     = data.aws_api_gateway_rest_api.this.id == run.prepare_test_setup.rest_api.id
    error_message = "Expected REST API ID to match the one from test setup."
  }
}
