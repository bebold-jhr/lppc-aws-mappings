...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_sfn_state_machine"
  }
...
run "fetch_state_machine" {
  ...
  variables {
    state_machine_name = run.prepare_test_setup.state_machine.name
  }
  ...
  assert {
    condition     = data.aws_sfn_state_machine.this.name == run.prepare_test_setup.state_machine.name
    error_message = "Expected state machine name to match the one from test setup."
  }
}
