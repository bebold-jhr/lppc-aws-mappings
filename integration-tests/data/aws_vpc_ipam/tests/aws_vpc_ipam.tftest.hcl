...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpc_ipam"
  }
...
run "fetch_vpc_ipam" {
  ...
  variables {
    ipam_id = run.prepare_test_setup.ipam.id
  }
  ...
  assert {
    condition     = data.aws_vpc_ipam.this.id == run.prepare_test_setup.ipam.id
    error_message = "Expected IPAM ID to match the one from test setup."
  }

  assert {
    condition     = data.aws_vpc_ipam.this.id != ""
    error_message = "Expected IPAM ID to not be empty."
  }
}
