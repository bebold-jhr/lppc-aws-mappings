...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_codeartifact_repository_endpoint"
  }
...
run "fetch_repository_endpoint" {
  ...
  variables {
    domain     = run.prepare_test_setup.domain.domain
    repository = run.prepare_test_setup.repository.repository
  }
  ...
  assert {
    condition     = data.aws_codeartifact_repository_endpoint.this.repository_endpoint != ""
    error_message = "Expected repository endpoint to be non-empty."
  }
}
