...
run "fetch_inference_profile" {
  ...
  assert {
    condition     = data.aws_bedrock_inference_profile.this.inference_profile_arn != ""
    error_message = "Expected inference profile to have a non-empty ARN."
  }
}
