...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_appconfig_configuration_profile"
  }
...
# Perform tests
####
run "fetch_configuration_profile" {
  state_key = "main"
  ...
  variables {
    ...
    configuration_profile_id = run.prepare_test_setup.configuration_profile.configuration_profile_id
  }
  ...
  assert {
    condition     = data.aws_appconfig_configuration_profile.this.arn == run.prepare_test_setup.configuration_profile.arn
    error_message = "Expected configuration profile ARN to match the one from test setup."
  }

  assert {
    condition     = data.aws_appconfig_configuration_profile.this.name == run.prepare_test_setup.configuration_profile.name
    error_message = "Expected configuration profile name to match the one from test setup."
  }
}
