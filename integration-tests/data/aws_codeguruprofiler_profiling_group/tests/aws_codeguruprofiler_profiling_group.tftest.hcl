...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_codeguruprofiler_profiling_group"
  }
...
run "fetch_profiling_group" {
  ...
  variables {
    profiling_group_name = run.prepare_test_setup.profiling_group.name
  }
  ...
  assert {
    condition     = data.aws_codeguruprofiler_profiling_group.this.name == run.prepare_test_setup.profiling_group.name
    error_message = "Expected profiling group name to match the one from test setup."
  }
}
