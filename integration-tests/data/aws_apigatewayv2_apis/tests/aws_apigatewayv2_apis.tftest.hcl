...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_apigatewayv2_apis"
  }
...
run "fetch_apis" {
  ...
  variables {
    api_name = run.prepare_test_setup.api.name
  }
  ...
  assert {
    condition     = contains(data.aws_apigatewayv2_apis.this.ids, run.prepare_test_setup.api.id)
    error_message = "Expected APIs to contain the API from test setup."
  }
}
