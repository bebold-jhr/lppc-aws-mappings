...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cloudfront_origin_access_identities"
  }
...
# Perform tests
####
run "fetch_origin_access_identities" {
  state_key = "main"
  ...
  variables {
    comments = [run.prepare_test_setup.origin_access_identity.comment]
  }
  ...
  assert {
    condition     = contains(data.aws_cloudfront_origin_access_identities.this.ids, run.prepare_test_setup.origin_access_identity.id)
    error_message = "Expected the created OAI ID to be in the returned identities."
  }
}
