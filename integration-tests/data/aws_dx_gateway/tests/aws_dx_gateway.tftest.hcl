...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_dx_gateway"
  }
...
run "fetch_dx_gateway" {
  ...
  variables {
    gateway_name = run.prepare_test_setup.dx_gateway.name
  }
  ...
  assert {
    condition     = data.aws_dx_gateway.this.id == run.prepare_test_setup.dx_gateway.id
    error_message = "Expected Direct Connect gateway ID to match the one from test setup."
  }
}
