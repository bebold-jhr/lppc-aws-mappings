...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_api_gateway_resource"
  }
...
run "fetch_resource" {
  ...
  variables {
    ...
    path        = run.prepare_test_setup.resource.path
  }
  ...
  assert {
    condition     = data.aws_api_gateway_resource.this.id == run.prepare_test_setup.resource.id
    error_message = "Expected resource ID to match the one from test setup."
  }
}
