...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_organizations_organizational_units"
  }
...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::${var.management_account_id}:role/lppc/DataAwsOrganizationsOrganizationalUnits"
  }
}
...
run "fetch_organizational_units" {
  ...
  assert {
    ...
    condition     = startswith(data.aws_caller_identity.this.arn, "arn:aws:sts::${var.management_account_id}:assumed-role/DataAwsOrganizationsOrganizationalUnits")
  ...
  assert {
    condition     = length(data.aws_organizations_organizational_units.this) > 0
    error_message = "Expected result to contain at least 1 organizational unit."
  }
}
