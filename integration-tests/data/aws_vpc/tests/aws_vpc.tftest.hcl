...
run "fetch_vpc" {
  ...
  variables {
    vpc_id = run.prepare_test_setup.vpc.id
  }
  ...
  assert {
    condition     = data.aws_vpc.this.id == run.prepare_test_setup.vpc.id
    error_message = "Expected VPC ID to match the one from test setup."
  }

  assert {
    condition     = data.aws_vpc.this.cidr_block == "10.0.0.0/16"
    error_message = "Expected VPC CIDR block to be 10.0.0.0/16."
  }
}
