...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpc_dhcp_options"
  }
...
run "fetch_dhcp_options" {
  ...
  variables {
    dhcp_options_id = run.prepare_test_setup.dhcp_options.id
  }
  ...
  assert {
    condition     = data.aws_vpc_dhcp_options.this.dhcp_options_id == run.prepare_test_setup.dhcp_options.id
    error_message = "Expected DHCP options ID to match the one from test setup."
  }

  assert {
    condition     = data.aws_vpc_dhcp_options.this.domain_name == "example.com"
    error_message = "Expected domain name to be example.com."
  }
}
