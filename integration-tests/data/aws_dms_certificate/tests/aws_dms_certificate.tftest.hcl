...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_dms_certificate"
  }
...
run "fetch_dms_certificate" {
  ...
  variables {
    certificate_id = run.prepare_test_setup.certificate.certificate_id
  }
  ...
  assert {
    condition     = data.aws_dms_certificate.this.certificate_arn == run.prepare_test_setup.certificate.certificate_arn
    error_message = "Expected certificate ARN to match the one from test setup."
  }
}
