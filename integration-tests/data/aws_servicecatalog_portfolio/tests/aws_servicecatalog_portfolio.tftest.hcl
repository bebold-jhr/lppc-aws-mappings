...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_servicecatalog_portfolio"
  }
...
run "fetch_portfolio" {
  ...
  assert {
    condition     = data.aws_servicecatalog_portfolio.this.id == run.prepare_test_setup.portfolio.id
    error_message = "Expected portfolio ID to match the one from test setup."
  }
}
