...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_lb_listener"
  }
...
run "fetch_lb_listener" {
  ...
  variables {
    listener_arn = run.prepare_test_setup.listener.arn
  }
  ...
  assert {
    condition     = data.aws_lb_listener.this.arn == run.prepare_test_setup.listener.arn
    error_message = "Expected listener ARN to match the one from test setup."
  }
}
