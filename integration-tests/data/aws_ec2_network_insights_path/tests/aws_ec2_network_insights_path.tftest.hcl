...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ec2_network_insights_path"
  }
...
# Perform tests
####
run "fetch_network_insights_path" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_ec2_network_insights_path.this.id == var.path_id
    error_message = "Expected Network Insights Path ID to match the one from test setup."
  }
}
