...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_networkfirewall_firewall_policy"
  }
...
run "fetch_firewall_policy" {
  ...
  variables {
    firewall_policy_arn = run.prepare_test_setup.firewall_policy.arn
  }
  ...
  assert {
    condition     = data.aws_networkfirewall_firewall_policy.this.arn == run.prepare_test_setup.firewall_policy.arn
    error_message = "Expected firewall policy ARN to match."
  }
}
