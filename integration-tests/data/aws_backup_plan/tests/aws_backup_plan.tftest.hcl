...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_backup_plan"
  }
...
run "fetch_backup_plan" {
  ...
  variables {
    plan_id = run.prepare_test_setup.backup_plan.id
  }
  ...
  assert {
    condition     = data.aws_backup_plan.this.name == run.prepare_test_setup.backup_plan.name
    error_message = "Expected backup plan name to match."
  }
}
