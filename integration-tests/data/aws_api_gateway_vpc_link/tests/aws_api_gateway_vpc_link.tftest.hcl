...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_api_gateway_vpc_link"
  }
...
run "fetch_vpc_link" {
  ...
  variables {
    vpc_link_name = run.prepare_test_setup.vpc_link.name
  }
  ...
  assert {
    condition     = data.aws_api_gateway_vpc_link.this.id == run.prepare_test_setup.vpc_link.id
    error_message = "Expected VPC Link ID to match the one from test setup."
  }
}
