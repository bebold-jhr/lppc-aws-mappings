...
run "fetch_patch_baselines" {
  ...
  assert {
    condition     = length(data.aws_ssm_patch_baselines.this.baseline_identities) > 0
    error_message = "Expected at least one AWS-owned patch baseline."
  }
}
