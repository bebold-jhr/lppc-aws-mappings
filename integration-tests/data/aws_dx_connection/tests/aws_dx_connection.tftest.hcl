...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_dx_connection"
  }
...
run "fetch_dx_connection" {
  ...
  variables {
    connection_name = run.prepare_test_setup.dx_connection.name
  }
  ...
  assert {
    condition     = data.aws_dx_connection.this.id == run.prepare_test_setup.dx_connection.id
    error_message = "Expected Direct Connect connection ID to match the one from test setup."
  }
}
