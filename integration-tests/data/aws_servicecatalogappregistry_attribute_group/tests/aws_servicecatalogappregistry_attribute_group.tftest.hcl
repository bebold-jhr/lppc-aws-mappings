...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_servicecatalogappregistry_attribute_group"
  }
...
run "fetch_attribute_group" {
  ...
  variables {
    attribute_group_id = run.prepare_test_setup.attribute_group.id
  }
  ...
  assert {
    condition     = data.aws_servicecatalogappregistry_attribute_group.this.arn != ""
    error_message = "Expected attribute group ARN to not be empty."
  }
}
