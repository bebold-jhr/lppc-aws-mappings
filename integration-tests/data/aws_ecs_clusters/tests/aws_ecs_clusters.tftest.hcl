...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ecs_clusters"
  }
...
run "fetch_clusters" {
  ...
  assert {
    condition     = contains(data.aws_ecs_clusters.this.cluster_arns, run.prepare_test_setup.cluster.arn)
    error_message = "Expected the created cluster ARN to appear in the list of clusters."
  }
}
