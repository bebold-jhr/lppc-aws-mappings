...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ec2_transit_gateway_route_table_associations"
  }
...
# Perform tests
####
run "fetch_transit_gateway_route_table_associations" {
  state_key = "main"
  ...
  assert {
    condition     = length(data.aws_ec2_transit_gateway_route_table_associations.this.ids) > 0
    error_message = "Expected at least one route table association."
  }
}
