...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_api_gateway_export"
  }
...
run "fetch_export" {
  ...
  variables {
    ...
    stage_name  = run.prepare_test_setup.stage.stage_name
  }
  ...
  assert {
    condition     = data.aws_api_gateway_export.this.body != null
    error_message = "Expected export body to not be null."
  }
}
