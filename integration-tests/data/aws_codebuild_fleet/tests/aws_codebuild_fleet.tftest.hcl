...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_codebuild_fleet"
  }
...
run "fetch_fleet" {
  ...
  variables {
    fleet_name = run.prepare_test_setup.fleet.name
  }
  ...
  assert {
    condition     = data.aws_codebuild_fleet.this.name == run.prepare_test_setup.fleet.name
    error_message = "Expected fleet name to match the one from test setup."
  }
}
