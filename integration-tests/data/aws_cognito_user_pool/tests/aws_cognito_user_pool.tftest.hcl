...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cognito_user_pool"
  }
...
run "fetch_cognito_user_pool" {
  ...
  assert {
    condition     = data.aws_cognito_user_pool.this.arn == run.prepare_test_setup.user_pool.arn
    error_message = "Expected user pool ARN to match the one from test setup."
  }
}
