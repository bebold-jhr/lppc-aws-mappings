...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_internet_gateway"
  }
...
run "fetch_internet_gateway" {
  ...
  variables {
    internet_gateway_id = run.prepare_test_setup.internet_gateway.id
  }
  ...
  assert {
    condition     = data.aws_internet_gateway.this.id == run.prepare_test_setup.internet_gateway.id
    error_message = "Expected IGW ID to match."
  }
}
