...
run "fetch_elb_service_account" {
  ...
  assert {
    condition     = data.aws_elb_service_account.this.id != ""
    error_message = "Expected ELB service account ID to not be empty."
  }

  assert {
    condition     = startswith(data.aws_elb_service_account.this.arn, "arn:aws:iam::")
    error_message = "Expected ELB service account ARN to be a valid IAM ARN."
  }
}
