...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_autoscaling_group"
  }
...
run "fetch_autoscaling_group" {
  ...
  variables {
    name = run.prepare_test_setup.autoscaling_group.name
  }
  ...
  assert {
    condition     = data.aws_autoscaling_group.this.name == run.prepare_test_setup.autoscaling_group.name
    error_message = "Expected autoscaling group name to match the one from test setup."
  }
}
