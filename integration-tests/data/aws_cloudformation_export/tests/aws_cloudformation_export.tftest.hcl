...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cloudformation_export"
  }
...
# Perform tests
####
run "fetch_cloudformation_export" {
  state_key = "main"
  ...
  variables {
    export_name = run.prepare_test_setup.export_name
  }
  ...
  assert {
    condition     = data.aws_cloudformation_export.this.value == "test-export-value"
    error_message = "Expected export value to be 'test-export-value'."
  }
}
