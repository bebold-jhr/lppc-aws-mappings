...
# Perform tests
####
run "fetch_log_delivery_canonical_user_id" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_cloudfront_log_delivery_canonical_user_id.this.id != ""
    error_message = "Expected canonical user ID to be non-empty."
  }
}
