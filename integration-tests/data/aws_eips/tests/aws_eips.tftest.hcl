...
run "fetch_eips" {
  ...
  variables {
    eip_name = run.prepare_test_setup.eip.tags_all.Name
  }
  ...
  assert {
    condition     = length(data.aws_eips.this.allocation_ids) > 0
    error_message = "Expected at least one EIP to be returned."
  }
}
