...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_organizations_organizational_unit_child_accounts"
  }
...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::${var.management_account_id}:role/lppc/DataAwsOrganizationsOrganizationalUnitChildAccounts"
  }
}
...
run "fetch_accounts" {
  ...
  assert {
    ...
    condition     = startswith(data.aws_caller_identity.this.arn, "arn:aws:sts::${var.management_account_id}:assumed-role/DataAwsOrganizationsOrganizationalUnitChildAccounts")
  ...
  assert {
    condition     = data.aws_organizations_organizational_unit_child_accounts.this.accounts != null && length(data.aws_organizations_organizational_unit_child_accounts.this.accounts) > 0
