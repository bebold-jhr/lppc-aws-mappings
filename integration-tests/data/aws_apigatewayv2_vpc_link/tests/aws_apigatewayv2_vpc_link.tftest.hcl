...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_apigatewayv2_vpc_link"
  }
...
run "fetch_vpc_link" {
  ...
  variables {
    vpc_link_id = run.prepare_test_setup.vpc_link.id
  }
  ...
  assert {
    condition     = data.aws_apigatewayv2_vpc_link.this.id == run.prepare_test_setup.vpc_link.id
