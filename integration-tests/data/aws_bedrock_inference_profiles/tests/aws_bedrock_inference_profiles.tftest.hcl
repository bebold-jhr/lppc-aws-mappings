...
run "fetch_inference_profiles" {
  ...
  assert {
    condition     = length(data.aws_bedrock_inference_profiles.this.inference_profile_summaries) > 0
    error_message = "Expected at least one inference profile."
  }
}
