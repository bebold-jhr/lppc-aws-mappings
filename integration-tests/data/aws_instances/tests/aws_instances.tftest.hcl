...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_instances"
  }
...
run "fetch_instances" {
  ...
  variables {
    instance_id = run.prepare_test_setup.instance.id
  }
  ...
  assert {
    condition     = length(data.aws_instances.this.ids) > 0
    error_message = "Expected at least one instance."
  }

  assert {
    condition     = contains(data.aws_instances.this.ids, run.prepare_test_setup.instance.id)
    error_message = "Expected instance from setup to be in the results."
  }
}
