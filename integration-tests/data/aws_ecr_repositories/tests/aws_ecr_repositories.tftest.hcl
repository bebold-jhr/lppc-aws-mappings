...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ecr_repositories"
  }
...
run "fetch_repositories" {
  ...
  assert {
    condition     = contains(data.aws_ecr_repositories.this.names, var.repository_name)
    error_message = "Expected ECR repositories list to contain the repository created in setup."
  }
}
