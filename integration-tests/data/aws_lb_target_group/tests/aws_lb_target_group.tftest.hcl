...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_lb_target_group"
  }
...
run "fetch_lb_target_group" {
  ...
  variables {
    target_group_name = run.prepare_test_setup.target_group.name
  }
  ...
  assert {
    condition     = data.aws_lb_target_group.this.arn == run.prepare_test_setup.target_group.arn
    error_message = "Expected target group ARN to match the one from test setup."
  }
}
