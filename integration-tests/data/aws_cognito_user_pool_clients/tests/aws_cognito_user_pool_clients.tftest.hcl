...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cognito_user_pool_clients"
  }
...
# Perform tests
####
run "fetch_cognito_user_pool_clients" {
  state_key = "main"
  ...
  assert {
    condition     = contains(data.aws_cognito_user_pool_clients.this.client_ids, run.prepare_test_setup.user_pool_client.id)
    error_message = "Expected user pool client ID to be in the results."
  }
}
