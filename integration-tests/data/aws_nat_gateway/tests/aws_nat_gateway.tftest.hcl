...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_nat_gateway"
  }
...
run "fetch_nat_gateway" {
  ...
  variables {
    nat_gateway_id = run.prepare_test_setup.nat_gateway.id
  }
  ...
  assert {
    condition     = data.aws_nat_gateway.this.id == run.prepare_test_setup.nat_gateway.id
    error_message = "Expected NAT Gateway ID to match the one from test setup."
  }
}
