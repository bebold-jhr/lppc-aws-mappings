...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_appconfig_application"
  }
...
run "fetch_application" {
  ...
  variables {
    application_name = run.prepare_test_setup.application.name
  }
  ...
  assert {
    condition     = data.aws_appconfig_application.this.id == run.prepare_test_setup.application.id
    error_message = "Expected application ID to match the one from test setup."
  }

  assert {
    condition     = data.aws_appconfig_application.this.arn == run.prepare_test_setup.application.arn
    error_message = "Expected application ARN to match the one from test setup."
  }
}
