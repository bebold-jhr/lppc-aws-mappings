...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_subnet"
  }
...
run "fetch_subnet" {
  ...
  variables {
    subnet_id = run.prepare_test_setup.subnet.id
  }
  ...
  assert {
    condition     = data.aws_subnet.this.id == run.prepare_test_setup.subnet.id
    error_message = "Expected subnet ID to match the one from test setup."
  }

  assert {
    condition     = data.aws_subnet.this.cidr_block == "10.0.1.0/24"
    error_message = "Expected subnet CIDR block to be 10.0.1.0/24."
  }
}
