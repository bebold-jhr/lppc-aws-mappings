...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_key_pair"
  }
...
run "fetch_key_pair" {
  ...
  variables {
    key_name = run.prepare_test_setup.key_pair.key_name
  }
  ...
  assert {
    condition     = data.aws_key_pair.this.key_name == run.prepare_test_setup.key_pair.key_name
    error_message = "Expected key pair name to match."
  }
}
