...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_network_interface"
  }
...
run "fetch_network_interface" {
  ...
  assert {
    condition     = data.aws_network_interface.this.id == run.prepare_test_setup.network_interface.id
    error_message = "Expected ENI ID to match."
  }
}
