...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpc_ipam_pools"
  }
...
run "fetch_vpc_ipam_pools" {
  ...
  assert {
    condition     = length(data.aws_vpc_ipam_pools.this.ipam_pools) > 0
    error_message = "Expected at least one IPAM Pool to be returned."
  }
}
