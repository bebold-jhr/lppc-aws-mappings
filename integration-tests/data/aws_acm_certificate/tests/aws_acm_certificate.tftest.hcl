...
run "fetch_acm_certificate" {
  ...
  variables {
    domain = run.prepare_test_setup.certificate.domain_name
  }
  ...
  assert {
    condition     = data.aws_acm_certificate.this.arn == run.prepare_test_setup.certificate.arn
    error_message = "Expected ACM certificate ARN to match the one from test setup."
  }
}
