...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_batch_scheduling_policy"
  }
...
# Perform tests
####
run "fetch_batch_scheduling_policy" {
  state_key = "main"
  ...
  variables {
    scheduling_policy_arn = run.prepare_test_setup.scheduling_policy.arn
  }
  ...
  assert {
    condition     = data.aws_batch_scheduling_policy.this.arn == run.prepare_test_setup.scheduling_policy.arn
    error_message = "Expected scheduling policy ARN to match the one from test setup."
  }
}
