...
run "fetch_cluster_versions" {
  ...
  assert {
    condition     = length(data.aws_eks_cluster_versions.this.cluster_versions) > 0
    error_message = "Expected at least one EKS cluster version to be available."
  }
}
