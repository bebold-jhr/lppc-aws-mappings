...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ec2_transit_gateway_attachments"
  }
...
# Perform tests
####
run "fetch_transit_gateway_attachments" {
  state_key = "main"
  ...
  assert {
    condition     = length(data.aws_ec2_transit_gateway_attachments.this.ids) > 0
    error_message = "Expected at least one transit gateway attachment."
  }
}
