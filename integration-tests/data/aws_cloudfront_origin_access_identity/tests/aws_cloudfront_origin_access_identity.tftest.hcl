...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cloudfront_origin_access_identity"
  }
...
# Perform tests
####
run "fetch_origin_access_identity" {
  state_key = "main"
  ...
  variables {
    id = run.prepare_test_setup.origin_access_identity.id
  }
  ...
  assert {
    condition     = data.aws_cloudfront_origin_access_identity.this.comment == run.prepare_test_setup.origin_access_identity.comment
    error_message = "Expected OAI comment to match the one from test setup."
  }
}
