...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpclattice_service"
  }
...
run "fetch_vpclattice_service" {
  ...
  assert {
    condition     = data.aws_vpclattice_service.this.id == run.prepare_test_setup.service.id
    error_message = "Expected VPC Lattice service ID to match the one from test setup."
  }
}
