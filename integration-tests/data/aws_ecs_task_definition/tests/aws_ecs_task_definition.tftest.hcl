...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ecs_task_definition"
  }
...
run "fetch_task_definition" {
  ...
  variables {
    task_definition_family = run.prepare_test_setup.task_definition.family
  }
  ...
  assert {
    condition     = data.aws_ecs_task_definition.this.arn == run.prepare_test_setup.task_definition.arn
    error_message = "Expected task definition ARN to match the one from test setup."
  }
}
