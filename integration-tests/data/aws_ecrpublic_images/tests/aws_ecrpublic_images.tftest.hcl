...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ecrpublic_images"
  }
...
run "fetch_images" {
  ...
  assert {
    condition     = data.aws_ecrpublic_images.this.images == null || length(data.aws_ecrpublic_images.this.images) >= 0
    error_message = "Expected ecrpublic images data source to execute successfully."
  }
}
