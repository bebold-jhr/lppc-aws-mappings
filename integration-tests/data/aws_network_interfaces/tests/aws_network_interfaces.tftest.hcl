...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_network_interfaces"
  }
...
run "fetch_network_interfaces" {
  ...
  variables {
    network_interface_id = run.prepare_test_setup.network_interface.id
  }
  ...
  assert {
    condition     = length(data.aws_network_interfaces.this.ids) > 0
    error_message = "Expected at least one network interface."
  }

  assert {
    condition     = contains(data.aws_network_interfaces.this.ids, run.prepare_test_setup.network_interface.id)
    error_message = "Expected ENI from setup to be in the results."
  }
}
