...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_dynamodb_table"
  }
...
run "fetch_dynamodb_table" {
  ...
  assert {
    condition     = data.aws_dynamodb_table.this.name == run.prepare_test_setup.dynamodb_table.name
    error_message = "Expected DynamoDB table name to match the one from test setup."
  }
}
