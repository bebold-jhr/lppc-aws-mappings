...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ec2_transit_gateway_attachment"
  }
...
# Perform tests
####
run "fetch_transit_gateway_attachment" {
  state_key = "main"
  ...
  variables {
    attachment_id      = run.prepare_test_setup.vpc_attachment.id
    transit_gateway_id = run.prepare_test_setup.transit_gateway.id
  }
  ...
  assert {
    condition     = data.aws_ec2_transit_gateway_attachment.this.resource_type == "vpc"
    error_message = "Expected attachment resource type to be vpc."
  }

  assert {
    condition     = data.aws_ec2_transit_gateway_attachment.this.transit_gateway_id == var.transit_gateway_id
    error_message = "Expected Transit Gateway ID to match."
  }
}
