...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_auditmanager_framework"
  }
...
# Perform tests
####
run "fetch_auditmanager_framework" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_auditmanager_framework.this.id != ""
    error_message = "Expected Audit Manager framework ID to not be empty."
  }

  assert {
    condition     = data.aws_auditmanager_framework.this.framework_type == "Standard"
    error_message = "Expected Audit Manager framework type to be Standard."
  }
}
