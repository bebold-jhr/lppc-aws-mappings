...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_codestarconnections_connection"
  }
...
run "fetch_connection" {
  ...
  variables {
    connection_name = run.prepare_test_setup.connection.name
  }
  ...
  assert {
    condition     = data.aws_codestarconnections_connection.this.name == run.prepare_test_setup.connection.name
    error_message = "Expected connection name to match the one from test setup."
  }
}
