...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_globalaccelerator_custom_routing_accelerator"
  }
...
# Perform tests
####
run "fetch_globalaccelerator_custom_routing_accelerator" {
  state_key = "main"
  ...
  variables {
    accelerator_name = run.prepare_test_setup.accelerator.name
  }
  ...
  assert {
    condition     = data.aws_globalaccelerator_custom_routing_accelerator.this.name == run.prepare_test_setup.accelerator.name
    error_message = "Expected custom routing accelerator name to match the one from test setup."
  }
}
