...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cognito_user_pool_client"
  }
...
# Perform tests
####
run "fetch_cognito_user_pool_client" {
  state_key = "main"
  ...
  variables {
    client_id    = run.prepare_test_setup.user_pool_client.id
    user_pool_id = run.prepare_test_setup.user_pool.id
  }
  ...
  assert {
    condition     = data.aws_cognito_user_pool_client.this.name == run.prepare_test_setup.user_pool_client.name
    error_message = "Expected user pool client name to match."
  }
}
