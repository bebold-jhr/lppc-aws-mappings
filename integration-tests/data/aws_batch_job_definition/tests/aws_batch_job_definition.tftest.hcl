...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_batch_job_definition"
  }
...
# Perform tests
####
run "fetch_batch_job_definition" {
  state_key = "main"
  ...
  variables {
    job_definition_name = run.prepare_test_setup.job_definition.name
  }
  ...
  assert {
    condition     = data.aws_batch_job_definition.this.name == run.prepare_test_setup.job_definition.name
    error_message = "Expected job definition name to match the one from test setup."
  }
}
