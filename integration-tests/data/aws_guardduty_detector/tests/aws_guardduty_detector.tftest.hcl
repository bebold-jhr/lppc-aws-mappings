...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_guardduty_detector"
  }
...
run "fetch_guardduty_detector" {
  ...
  variables {
    detector_id = run.prepare_test_setup.detector.id
  }
  ...
  assert {
    condition     = data.aws_guardduty_detector.this.id == run.prepare_test_setup.detector.id
    error_message = "Expected GuardDuty detector ID to match the one from test setup."
  }
}
