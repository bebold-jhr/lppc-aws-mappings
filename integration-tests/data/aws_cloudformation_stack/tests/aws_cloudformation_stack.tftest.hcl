...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cloudformation_stack"
  }
...
# Perform tests
####
run "fetch_cloudformation_stack" {
  state_key = "main"
  ...
  variables {
    stack_name = run.prepare_test_setup.stack.name
  }
  ...
  assert {
    condition     = data.aws_cloudformation_stack.this.outputs["TestOutput"] == "test-output-value"
    error_message = "Expected stack output 'TestOutput' to be 'test-output-value'."
  }
}
