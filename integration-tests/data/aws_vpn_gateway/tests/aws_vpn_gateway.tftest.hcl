...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpn_gateway"
  }
...
run "fetch_vpn_gateway" {
  ...
  variables {
    vpn_gateway_id = run.prepare_test_setup.vpn_gateway.id
  }
  ...
  assert {
    condition     = data.aws_vpn_gateway.this.id == run.prepare_test_setup.vpn_gateway.id
    error_message = "Expected VPN gateway ID to match."
  }
}
