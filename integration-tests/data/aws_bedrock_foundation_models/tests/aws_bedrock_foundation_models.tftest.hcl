...
run "fetch_foundation_models" {
  ...
  assert {
    condition     = length(data.aws_bedrock_foundation_models.this.model_summaries) > 0
    error_message = "Expected at least one foundation model."
  }
}
