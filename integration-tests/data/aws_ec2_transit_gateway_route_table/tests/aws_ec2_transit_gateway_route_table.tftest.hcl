...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_ec2_transit_gateway_route_table"
  }
...
# Perform tests
####
run "fetch_transit_gateway_route_table" {
  state_key = "main"
  ...
  variables {
    transit_gateway_route_table_id = run.prepare_test_setup.transit_gateway.association_default_route_table_id
  ...
  assert {
    condition     = data.aws_ec2_transit_gateway_route_table.this.transit_gateway_id == var.transit_gateway_id
