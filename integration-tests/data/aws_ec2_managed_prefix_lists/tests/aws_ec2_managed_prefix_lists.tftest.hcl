...
# Perform tests
####
run "fetch_managed_prefix_lists" {
  state_key = "main"
  ...
  assert {
    condition     = length(data.aws_ec2_managed_prefix_lists.this.ids) > 0
    error_message = "Expected prefix list to contain at least one entry."
  }
}
