...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpc_ipam_pool_cidrs"
  }
...
run "fetch_vpc_ipam_pool_cidrs" {
  ...
  variables {
    ipam_pool_id = run.prepare_test_setup.ipam_pool.id
  }
  ...
  assert {
    condition     = length(data.aws_vpc_ipam_pool_cidrs.this.ipam_pool_cidrs) > 0
    error_message = "Expected at least one IPAM Pool CIDR to be returned."
  }
}
