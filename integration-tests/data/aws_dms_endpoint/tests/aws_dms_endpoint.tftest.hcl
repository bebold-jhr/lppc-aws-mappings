...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_dms_endpoint"
  }
...
run "fetch_dms_endpoint" {
  ...
  variables {
    endpoint_id = run.prepare_test_setup.endpoint.endpoint_id
  }
  ...
  assert {
    condition     = data.aws_dms_endpoint.this.endpoint_arn == run.prepare_test_setup.endpoint.endpoint_arn
    error_message = "Expected endpoint ARN to match the one from test setup."
  }
}
