...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_servicecatalogappregistry_attribute_group_associations"
  }
...
# Perform tests
####
run "fetch_attribute_group_associations" {
  state_key = "main"
  ...
  assert {
    condition     = length(data.aws_servicecatalogappregistry_attribute_group_associations.this.attribute_group_ids) > 0
    error_message = "Expected at least one attribute group association."
  }
}
