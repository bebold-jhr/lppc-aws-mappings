...
run "fetch_elb" {
  ...
  variables {
    elb_name = run.prepare_test_setup.elb.name
  }
  ...
  assert {
    condition     = data.aws_elb.this.name == run.prepare_test_setup.elb.name
    error_message = "Expected ELB name to match the one from test setup."
  }
}
