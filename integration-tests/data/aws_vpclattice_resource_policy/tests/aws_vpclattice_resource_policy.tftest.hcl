...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpclattice_resource_policy"
  }
...
# Perform tests
####
run "fetch_vpclattice_resource_policy" {
  state_key = "main"
  ...
  variables {
    resource_arn = run.prepare_test_setup.service_network.arn
  }
  ...
  assert {
    condition     = data.aws_vpclattice_resource_policy.this.policy != ""
    error_message = "Expected VPC Lattice resource policy to not be empty."
  }
}
