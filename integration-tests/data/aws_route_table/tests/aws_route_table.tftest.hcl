...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_route_table"
  }
...
run "fetch_route_table" {
  ...
  variables {
    route_table_id = run.prepare_test_setup.route_table.id
  }
  ...
  assert {
    condition     = data.aws_route_table.this.route_table_id == run.prepare_test_setup.route_table.id
    error_message = "Expected route table ID to match the one from test setup."
  }

  assert {
    condition     = data.aws_route_table.this.vpc_id == run.prepare_test_setup.vpc.id
    error_message = "Expected VPC ID to match the one from test setup."
  }
}
