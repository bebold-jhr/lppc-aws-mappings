...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_wafv2_ip_set"
  }
...
run "fetch_ip_set" {
  ...
  variables {
    name = run.prepare_test_setup.ip_set.name
  }
  ...
  assert {
    condition     = data.aws_wafv2_ip_set.this.name == run.prepare_test_setup.ip_set.name
    error_message = "Expected WAFv2 IP set name to match the one from test setup."
  }
}
