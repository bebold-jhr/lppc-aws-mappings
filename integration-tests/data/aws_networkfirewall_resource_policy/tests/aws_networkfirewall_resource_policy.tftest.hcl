...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_networkfirewall_resource_policy"
  }
...
run "fetch_resource_policy" {
  ...
  variables {
    resource_arn = run.prepare_test_setup.firewall_policy.arn
  }
  ...
  assert {
    condition     = data.aws_networkfirewall_resource_policy.this.resource_arn == run.prepare_test_setup.firewall_policy.arn
    error_message = "Expected resource ARN to match."
  }

  assert {
    condition     = data.aws_networkfirewall_resource_policy.this.policy != ""
    error_message = "Expected policy to not be empty."
  }
}
