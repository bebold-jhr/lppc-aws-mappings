...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_customer_gateway"
  }
...
run "fetch_customer_gateway" {
  ...
  variables {
    customer_gateway_id = run.prepare_test_setup.customer_gateway.id
  }
  ...
  assert {
    condition     = data.aws_customer_gateway.this.id == run.prepare_test_setup.customer_gateway.id
    error_message = "Expected customer gateway ID to match the one from test setup."
  }

  assert {
    condition     = data.aws_customer_gateway.this.ip_address == "172.83.124.10"
    error_message = "Expected IP address to be 172.83.124.10."
  }
}
