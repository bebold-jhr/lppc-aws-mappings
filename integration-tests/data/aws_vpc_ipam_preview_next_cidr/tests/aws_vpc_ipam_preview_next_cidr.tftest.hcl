...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_vpc_ipam_preview_next_cidr"
  }
...
# Perform tests
####
run "fetch_vpc_ipam_preview_next_cidr" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_vpc_ipam_preview_next_cidr.this.cidr != ""
    error_message = "Expected preview next CIDR to not be empty."
  }
}
