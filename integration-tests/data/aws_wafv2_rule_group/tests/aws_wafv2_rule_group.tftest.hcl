...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_wafv2_rule_group"
  }
...
run "fetch_rule_group" {
  ...
  variables {
    name = run.prepare_test_setup.rule_group.name
  }
  ...
  assert {
    condition     = data.aws_wafv2_rule_group.this.name == run.prepare_test_setup.rule_group.name
    error_message = "Expected WAFv2 rule group name to match the one from test setup."
  }
}
