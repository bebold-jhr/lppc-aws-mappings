...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cloudfront_origin_access_control"
  }
...
# Perform tests
####
run "fetch_origin_access_control" {
  state_key = "main"
  ...
  variables {
    id = run.prepare_test_setup.origin_access_control.id
  }
  ...
  assert {
    condition     = data.aws_cloudfront_origin_access_control.this.name == run.prepare_test_setup.origin_access_control.name
    error_message = "Expected OAC name to match."
  }
}
