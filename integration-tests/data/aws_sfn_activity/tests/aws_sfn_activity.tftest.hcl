...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_sfn_activity"
  }
...
run "fetch_activity" {
  ...
  variables {
    activity_name = run.prepare_test_setup.activity.name
  }
  ...
  assert {
    condition     = data.aws_sfn_activity.this.name == run.prepare_test_setup.activity.name
    error_message = "Expected activity name to match the one from test setup."
  }
}
