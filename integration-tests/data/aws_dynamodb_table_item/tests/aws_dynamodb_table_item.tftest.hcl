...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_dynamodb_table_item"
  }
...
run "fetch_dynamodb_table_item" {
  ...
  variables {
    table_name = run.prepare_test_setup.dynamodb_table.name
    key = jsonencode({
      "id" = { "S" = jsondecode(run.prepare_test_setup.dynamodb_table_item.item).id.S }
    })
  }
  ...
  assert {
    condition     = data.aws_dynamodb_table_item.this.item != null
    error_message = "Expected DynamoDB table item to be returned."
  }
}
