...
run "fetch_lb" {
  ...
  variables {
    lb_name = run.prepare_test_setup.lb.name
  }
  ...
  assert {
    condition     = data.aws_lb.this.arn == run.prepare_test_setup.lb.arn
    error_message = "Expected load balancer ARN to match the one from test setup."
  }
}
