...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_api_gateway_authorizer"
  }
...
run "fetch_authorizer" {
  ...
  variables {
    rest_api_id   = run.prepare_test_setup.rest_api.id
    authorizer_id = run.prepare_test_setup.authorizer.id
  }
  ...
  assert {
    condition     = data.aws_api_gateway_authorizer.this.name == run.prepare_test_setup.authorizer.name
    error_message = "Expected authorizer name to match the one from test setup."
  }
}
