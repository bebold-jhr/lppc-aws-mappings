...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_location_geofence_collection"
  }
...
run "fetch_geofence_collection" {
  ...
  variables {
    collection_name = run.prepare_test_setup.geofence_collection.collection_name
  }
  ...
  assert {
    condition     = data.aws_location_geofence_collection.this.collection_name == run.prepare_test_setup.geofence_collection.collection_name
    error_message = "Expected geofence collection name to match the one from test setup."
  }
}
