...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_codeartifact_authorization_token"
  }
...
run "fetch_authorization_token" {
  ...
  assert {
    condition     = data.aws_codeartifact_authorization_token.this.authorization_token != ""
    error_message = "Expected authorization token to be non-empty."
  }
}
