...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cognito_user_pool_signing_certificate"
  }
...
# Perform tests
####
run "fetch_cognito_user_pool_signing_certificate" {
  state_key = "main"
  ...
  assert {
    condition     = data.aws_cognito_user_pool_signing_certificate.this.certificate != ""
    error_message = "Expected signing certificate to not be empty."
  }
}
