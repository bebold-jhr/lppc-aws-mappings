...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_wafv2_regex_pattern_set"
  }
...
run "fetch_regex_pattern_set" {
  ...
  variables {
    name = run.prepare_test_setup.regex_pattern_set.name
  }
  ...
  assert {
    condition     = data.aws_wafv2_regex_pattern_set.this.name == run.prepare_test_setup.regex_pattern_set.name
    error_message = "Expected WAFv2 regex pattern set name to match the one from test setup."
  }
}
