...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cloudfront_realtime_log_config"
  }
...
run "fetch_realtime_log_config" {
  ...
  variables {
    name = run.prepare_test_setup.realtime_log_config.name
  }
  ...
  assert {
    condition     = data.aws_cloudfront_realtime_log_config.this.arn == run.prepare_test_setup.realtime_log_config.arn
    error_message = "Expected realtime log config ARN to match the one from test setup."
  }
}
