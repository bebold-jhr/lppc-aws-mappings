...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cloudfront_function"
  }
...
run "fetch_function" {
  ...
  variables {
    name  = run.prepare_test_setup.function.name
    stage = "LIVE"
  }
  ...
  assert {
    condition     = data.aws_cloudfront_function.this.arn != ""
    error_message = "Expected function ARN to not be empty."
  }
}
