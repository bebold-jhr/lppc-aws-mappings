...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_sfn_alias"
  }
...
run "fetch_alias" {
  ...
  variables {
    alias_name       = run.prepare_test_setup.alias.name
    statemachine_arn = run.prepare_test_setup.state_machine.arn
  }
  ...
  assert {
    condition     = data.aws_sfn_alias.this.name == run.prepare_test_setup.alias.name
    error_message = "Expected alias name to match the one from test setup."
  }

  assert {
    condition     = data.aws_sfn_alias.this.arn == run.prepare_test_setup.alias.arn
    error_message = "Expected alias ARN to match the one from test setup."
  }
}
