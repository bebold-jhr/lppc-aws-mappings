...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_dynamodb_tables"
  }
...
run "fetch_dynamodb_tables" {
  ...
  assert {
    condition     = contains(data.aws_dynamodb_tables.this.names, run.prepare_test_setup.dynamodb_table.name)
    error_message = "Expected the created DynamoDB table to appear in the list of tables."
  }
}
