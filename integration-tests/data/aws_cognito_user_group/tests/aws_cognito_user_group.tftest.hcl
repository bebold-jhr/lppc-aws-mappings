...
run "prepare_test_setup" {
  ...
  module {
    source = "../../modules/data/aws_cognito_user_group"
  }
...
run "fetch_cognito_user_group" {
  ...
  variables {
    ...
    name         = run.prepare_test_setup.user_group.name
  }
  ...
  assert {
    condition     = data.aws_cognito_user_group.this.description == "Test group for integration test"
    error_message = "Expected user group description to match."
  }
}
